...
let VM = new Vue({
	...
	methods: {
		...
		chooseDataAll() { //			全选	
			...
			if(this.chooseAllCheckboxFlag == true) { //把所有的checkbox勾选上
				for(let i = 0; i < this.publicOrdersTableData.length; i++) {
					if(this.publicOrdersTableData[i] == "未处理") {
		...
		exportData() { //导出
			let params = `location_id=${this.location_id}&order_number=${this.order_number}&status=${this.status}&begindate=${this.begindate}&enddate=${this.enddate}&mid=${this.mid}`;
