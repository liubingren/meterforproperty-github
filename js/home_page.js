...
let hpVm = new Vue({
  ...
  methods:{
    ...
    getpersonalInfor(){
      ...
      let datalist
      axios({
        url: HEADER + '/home/check_userMessage.do',
      })
    ...
    getessentialInfor(){
      ...
      axios({
        url: HEADER + '/home/check_countMeter.do',
      })
        .then(function(response){
          ...
          for(let i = 0; i < dataArr.length; i++){
            ...
            THIS.legendList.push(dataArr[i].name)
          }
          // datalist = [
          //     {value:(!!dataArr[0].count ? dataArr[0].count : 0), name:'三相安规电表'},
          //     {value:(!!dataArr[1].count ? dataArr[1].count : 0), name:'单相电表'},
          //     {value:(!!dataArr[2].count ? dataArr[2].count : 0), name:'预付费电表'}
          //   ]
          THIS.drawLine(datalist)
        })
    },
    //画图
    drawLine(datalist,legendList){
      this.totalCharts = echarts.init(document.getElementById('total_charts'))
      let option = {
        color: ['#15c0e5', '#afe570','#E5D135','#ffa991'],
        tooltip: {
          trigger: 'item',
          formatter: "{b} : {c} ({d}%)"
        },
